...
void markDupsDiscordants(splitLine_t * block, state_t * state)
{
	...
	// by song
	if( off > state->sigArraySize)
	{
		//something wrong for calculating off. force to set off as 0
		off = 0;
		cnt_off_errors++;
        ...
        //by song
	//fprintf(stderr, "samblaster: off = %u, sigArraySize = %d\n", off, state->sigArraySize);
...
int main (int argc, char *argv[])
{
	...
	//by song
	if (!strcmp(nextLine->fields[0], "^!song")) break;
    ...
    fprintf(stderr, "samblaster: dedup process ends \n");
    ...
    // by song, print cnt_off_errors
    fprintf(stderr, "samblaster: Number of errors in calculating off %d\n", cnt_off_errors);
