...
int kclose(void *a);
void kt_pipeline(int n_threads, void *(*func)(void*, int, void*, void*), void *shared_data, int n_steps, void *_ptr);
...
static void *process(void *shared, int step, void *_data, void *_ptr)
{
		...
		const mem_opt_t *opt = aux->opt;
	       ...
	       zmq_connect (sender, opt->broker);

	       char *buf10 = (char*) malloc(sizeof(char)*bufsize);
	       memset(buf10,0,sizeof(char)*bufsize);

	       int base=0;
	       for (i = 1; i < size; ++i) {
		      if (data->seqs[i].sam) {
			     if(strcmp(data->seqs[i-1].name, data->seqs[i].name) != 0) 
			     {
				    to = i-1;
				    int in = 0;
				    int cnt = to - from;
				    ...
				    splitLine_t * firstLine  = NULL;
				    splitLine_t * secondLine = NULL;

				    char * tmp_string = (char*) malloc(sizeof(char)*3000);
				    char * tmp_string1 = (char*) malloc(sizeof(char)*3000);

				    splitLine_t * tmp_arr[3];
				    splitLine_t * tmp_arr1[3];

				    char * pnt = NULL;

				    strcpy(tmp_string1, data->seqs[from].sam);
				    char * tmp1 = tmp_string1;
				    int tmp_cnt1 = 0;

				    while((pnt = strsep(&tmp1,"\n")) != NULL) {
					   if(strcmp(pnt,"")!=0) {
						  tmp_arr1[tmp_cnt1] = s_splitNKey(pnt, ptr);
						  unsplitSplitLine(tmp_arr1[tmp_cnt1]);							
						  tmp_cnt1 += 1;
					   }
				    }
				    free(tmp_string1);

				    strcpy(tmp_string, data->seqs[to].sam);
				    char * tmp = tmp_string;

				    int tmp_cnt = 0;
				    while((pnt = strsep(&tmp,"\n")) != NULL) {
					   if(strcmp(pnt,"")!=0) {
						  //printf("strcmp : %s", pnt);
						  //printf("len:%d\n", strlen(pnt));
						  tmp_arr[tmp_cnt] = s_splitNKey(pnt, ptr);
						  unsplitSplitLine(tmp_arr[tmp_cnt]);							
						  tmp_cnt += 1;
					   }
				    }
				    free(tmp_string);

				    sgn_t key = calcSig(tmp_arr1[0], tmp_arr[0]);
				    sprintf(buf10+strlen(buf10),"%lluXXQQ", key);
				    printf("%llu\t", key);
				    printf("pos : %llu, binpos : %llu\n", tmp_arr1[0]->pos, tmp_arr1[0]->binPos); //delete this line
				    //sprintf(buf10+strlen(buf10),"%s@", firstLine->buffer);

				    int j;

				    for(j=0; j < tmp_cnt1; j++) {
					   //if((tmp_cnt1 - j) == 1) sprintf(buf10+strlen(buf10),"%s\n", tmp_arr1[j]->buffer);
					   //else sprintf(buf10+strlen(buf10),"%s@", tmp_arr1[j]->buffer);
					   sprintf(buf10+strlen(buf10),"%sXXQQ", tmp_arr1[j]->buffer);
					   deleteSplitLine(tmp_arr1[j]);
					   //if((tmp_cnt - i) == 1) printf("11:%s\n", tmp_arr[i]->buffer);
					   //else printf("22%s\n", tmp_arr[i]->buffer);
					   //deleteSplitLine(tmp_arr[i]);
				    }

				    for(j=0; j < tmp_cnt; j++) {
					   if((tmp_cnt - j) == 1) sprintf(buf10+strlen(buf10),"%s\n", tmp_arr[j]->buffer);
					   else sprintf(buf10+strlen(buf10),"%sXXQQ", tmp_arr[j]->buffer);
					   deleteSplitLine(tmp_arr[j]);
					   //if((tmp_cnt - i) == 1) printf("11:%s\n", tmp_arr[i]->buffer);
					   //else printf("22%s\n", tmp_arr[i]->buffer);
					   //deleteSplitLine(tmp_arr[i]);
				    }

				    if(strlen(buf10) > 512000) {
					   //printf("buf cleaned buf size(mem addr) : %ld\n", buf10+strlen(buf10));
					   //printf("buf cleaned buf size(strlen) : %ld\n", strlen(buf10));
					   //printf("mem setted size : %ld\n", sizeof(char)*1048576);
					   int sensize = s_send(sender,buf10);						
					   //printf("sendsize : %d / %d\n", sensize, strlen(buf10));
					   printf("%s", buf10);
                                           //zmq_term (context);
					   //printf("START1\n%sEND1",buf10);
					   memset(buf10,0,sizeof(char)*bufsize);
				    ...
				    //err_fputs(buf0, stdout);
				    from = i;
			     }
		      }
	       }
	       to = size-1;
	       if((to - from) > 2) { printf("size err : %d\n", to-from);}
	       ...
	       sgn_t key = calcSig(tmp_arr1[0], tmp_arr[0]);
	       sprintf(buf10+strlen(buf10),"%lldXXQQ", key);
	       int j;
	       for(j=0; j < tmp_cnt1; j++) {
		      sprintf(buf10+strlen(buf10),"%sXXQQ", tmp_arr1[j]->buffer);
		      deleteSplitLine(tmp_arr1[j]);
...
int main_mem(int argc, char *argv[], void * ptr)
{
        mem_opt_t *opt, opt0;
        ...
        s_bwa_print_sam_hdr(aux.idx->bns, hdr_line, ptr, opt->broker);
        aux.actual_chunk_size = fixed_chunk_size > 0? fixed_chunk_size : opt->chunk_size * opt->n_threads;
        kt_pipeline(no_mt_io? 1 : 2, process, &aux, 3, ptr);
        free(hdr_line);
        free(opt);
