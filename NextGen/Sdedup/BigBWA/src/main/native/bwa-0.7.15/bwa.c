...
void s_bwa_print_sam_hdr(const bntseq_t *bns, const char *hdr_line, void * ptr, char * broker)
{
  ...
  if (n_SQ == 0) {
    ...
    zmq_connect (sender, broker);


    for (i = 0; i < bns->n_seqs; ++i) {

      char buf[200];
      sprintf(buf,"@SQ\tSN:%s\tLN:%d\n", bns->anns[i].name, bns->anns[i].len);
      s_send(sender, buf);
      //printf("%s", buf);
      //err_printf("@SQ\tSN:%s\tLN:%d", bns->anns[i].name, bns->anns[i].len);
      s_setKey(bns->anns[i].name, bns->anns[i].len, ptr);
