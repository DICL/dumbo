...
int main(int argc, char *argv[]) 
{
	...
	if( argc < 3 ) {
		printf("Usage: mpirun -np <split count> -hostfile <hostfile> ./merge_bam prefix output_bam_file [first_file_id]\n");
		return -1;
	}
	prefix = argv[1];
	outFileName = argv[2];
	if( argc > 3 ) {
		fsoffset = atoi(argv[3]);
	}
	...
	sprintf(fileName,"%s%d.bam\0",prefix,rank + fsoffset);
