...
private[gpuenabler] class GPUMemoryManager(val executorId : String,
  ...
  val cachedCPUPointers = new HashMap[String, Seq[KernelParameterDesc]]()
  def getCachedCPUPointers = cachedCPUPointers

  val sizeOfEachCachedChunks = new HashMap[String, Int]()
  def getSizeOfChunk = sizeOfEachCachedChunks
  ...
  def cacheGPU(rddId : Int): Unit = {
    println("cacheGPU called at GPUMemoryManager")
  ...
  def cacheGPUSlaves(rddId : Int): Unit = {
    println("cacheGPUSlaves called at GPUMemoryManager")
