...
import org.apache.xbean.asm5.{ClassReader, ClassVisitor, MethodVisitor, _}
...
BB_RET:
    ...
    val cudaFunc = new CUDAFunction(fullName, Array("this"), Array("this"), resource = (fullName, ptxMap))
    Some(cudaFunc)
  }
...
BB_6:
  ...
  def generateForReduce[T: ClassTag](f: (T, T) => T): Option[CUDAFunction] = {
    ...
    val cudaFunc =
      new CUDAFunction(funcEntryName, Array("this"), Array("this"), resource = (funcEntryName, ptxReduce))
    Some(cudaFunc)
  }
}
