...
public interface Store extends HeapSize, StoreConfigInformation, PropagatingConfigurationObserver {
  ...
  /**
   * Added by seungwon
   * Return a scanner for both the memstore and the HStore files we want. Assumes we are not in a
   * compaction.
   * @param scan Scan to apply when scanning the stores
   * @param targetCols columns to scan
   * @param selectedStoreFiles StoreFileScanners we want
   * @return a scanner over the current key values
   * @throws IOException on failure
   */
  KeyValueScanner getScanner(Scan scan, final NavigableSet<byte[]> targetCols, long readPt, List<String> selectedStoreFiles)
      throws IOException;
   ...
   * @return all selected scanners for this store
   */
  List<KeyValueScanner> getScanners(
      boolean cacheBlocks,
      boolean isGet,
      ...
      byte[] stopRow,
      long readPt,
      List<String> selectedStoreFiles
    ) throws IOException;
