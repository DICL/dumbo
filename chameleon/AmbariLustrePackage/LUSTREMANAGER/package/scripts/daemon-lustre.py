...
class MyTCPHandler(SocketServer.BaseRequestHandler):
  def handle(self):
    ...
    elif(str(self.data)=='create_workingdir'):
      lustreSomething.create_workingdir()
    elif(str(self.data)=='remove_workingdir'):
      lustreSomething.remove_workingdir()
...
class LustreSomething:  
  ...
  # mount + create workingdir
  def create_workingdir(self):
    
    lustrefs_config = self.get_lustrefs_config();
    #mdt_fsname = "mylustre"
    #mds_host = "master"
    self.subprocess_open('echo "lustre_default_config '+str(lustrefs_config)+'" >> /tmp/dm-log.info')
    mds_host = lustrefs_config["LustreMDSMgmtService_host"];
    self.subprocess_open('echo "'+mds_host+'" >> /tmp/dm-log.info')
    mdt_fsname = lustrefs_config["lustrefs_mountpoint"];
    self.subprocess_open('echo "'+mdt_fsname+'" >> /tmp/dm-log.info')
    
    # 19.01.31 je.kim change mountpoint
    lustre_client_folder = lustrefs_config["lustrefs_mountpoint"];
    #lustre_client_folder = lustrefs_config["mount_client"];
    #lustre_client_folder = '/lustre1'
    
    self.subprocess_open('echo "'+lustre_client_folder+'" >> /tmp/dm-log.info')
    ...
    if len(str(check[1])) != 0:    
      ...
      for hostname in hosts:
        ...
        self.subprocess_open("ssh root@"+hostname+" -T \" chown yarn. /tmp2/hadoop-yarn \"")
        ...
        self.subprocess_open("ssh root@"+hostname+" -T \" chown yarn. /tmp2/yarn \"")
        ...
        # 190222 je.kim delete usercache files
        # Can't create directory /tmp2/yarn/local/usercache/user1/appcache/application_1550814111334_0001 - Permission denied
        self.subprocess_open("ssh root@"+hostname+" -T \" rm -rf /tmp2/yarn/local/usercache/* \"")
        pass # end for
      ...
      self.subprocess_open("mkdir -p /"+lustre_client_folder+"/hadoop/ats/active &> /tmp/yarn")
      self.subprocess_open("chown yarn. /"+lustre_client_folder+"/hadoop/ats &> /tmp/yarn")
      self.subprocess_open("chown yarn. /"+lustre_client_folder+"/hadoop/ats/active &> /tmp/yarn")
      ...
      self.subprocess_open(" mkdir -p /"+lustre_client_folder+"/hadoop/hdp/apps/2.6.3.0-235/mapreduce &>> /tmp/yarn")

      self.subprocess_open(" chown yarn. /"+lustre_client_folder+"/hadoop/hdp &>> /tmp/yarn")
      self.subprocess_open(" chown yarn. /"+lustre_client_folder+"/hadoop/hdp/apps &>> /tmp/yarn")
      self.subprocess_open(" chown yarn. /"+lustre_client_folder+"/hadoop/hdp/apps/2.6.3.0-235/ &>> /tmp/yarn")
      self.subprocess_open(" chown yarn. /"+lustre_client_folder+"/hadoop/hdp/apps/2.6.3.0-235/mapreduce &>> /tmp/yarn")
      self.subprocess_open(" cp /usr/hdp/2.6.3.0-235/hadoop/mapreduce.tar.gz /"+lustre_client_folder+"/hadoop/hdp/apps/2.6.3.0-235/mapreduce ")
      self.subprocess_open(" mkdir -p /"+lustre_client_folder+"/hadoop/mr-history/done &>> /tmp/mapred")
      self.subprocess_open(" mkdir -p /"+lustre_client_folder+"/hadoop/mr-history/tmp &>> /tmp/mapred")
      self.subprocess_open(" mkdir -p /"+lustre_client_folder+"/hadoop/mapred/system &>> /tmp/mapred")

      self.subprocess_open(" chown -R mapred. /"+lustre_client_folder+"/hadoop/mr-history &>> /tmp/mapred")
      self.subprocess_open(" chown -R mapred. /"+lustre_client_folder+"/hadoop/mapred &>> /tmp/mapred")
      pass # end if



    pass # end create_workingdir
  ...
  def remove_workingdir(self):
    ...
    #### mount ####
    check = commands.getstatusoutput('mount | grep /tmp2')
    if len(str(check[1])) != 0:    
      ...
      for hostname in hosts:
        self.subprocess_open("ssh root@"+hostname+" -T \"  umount -l /tmp2 &>> /tmp/yarn  \"")
        pass # end for
      pass # end if
  ...
  def getHostsInfo(self):
    ...
    for component in j["host_components"]:
      mds_host_list.append(component["HostRoles"]["host_name"])
    
    # je.kim oss 호스트정보 가져오기
    ...
    oss_host_list =[]
    for component in j["host_components"]:
      oss_host_list.append(component["HostRoles"]["host_name"])
    ...
    for component in j["host_components"]:
      client_host_name = component["HostRoles"]["host_name"]
      if not client_host_name in mds_host_list:
        if not client_host_name in oss_host_list:
          result.append(client_host_name)
    


    # self.subprocess_open('echo "'+str(len(result))+'" >> /tmp/test.info')
    return result
  ...
  def get_lustrefs_config(self):
    ...
    # 19.01.31 je.kim lustre view -> ambari config
    ...
    self.subprocess_open('echo "lustrefs_config_env '+str(lustrefs_config_env)+'" >> /tmp/dm-log.info')
  ...
  def umount(self):
    ...
    # lustre_client_folder = lustrefs_config["mount_client"];
    # 190131 je.kim 마운트 포인트 변경
    lustre_client_folder = lustrefs_config["lustrefs_mountpoint"];
    # lustre_client_folder = '/lustre'


    self.subprocess_open('echo "'+lustre_client_folder+'" >> /tmp/dm-log.info')

    check = commands.getstatusoutput('mount | grep lustre')
    self.subprocess_open('echo "'+str(check)+'" >> /tmp/dm-log.info')
    if len(str(check[1])) == 0:
