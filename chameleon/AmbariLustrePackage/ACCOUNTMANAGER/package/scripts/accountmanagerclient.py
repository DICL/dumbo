...
class ACCOUNTMANAGERCLIENT(Script):
  def __init__(self):
    self.sbtls = SubTools()
  ...
  def getMasterhost(self):
    ...
    r = requests.get('http://'+ambari_url+':8080/api/v1/clusters/'+cluster_name+'/services/ACCOUNTMANAGER/components/ACCOUNTMANAGERCLIENT', headers=headers, auth=('admin', 'admin'))
    j = json.loads(r.text)
    result =[]
