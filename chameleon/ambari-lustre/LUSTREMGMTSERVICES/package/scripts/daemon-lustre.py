...
class MyTCPHandler(SocketServer.BaseRequestHandler):
  def handle(self):
    ...
    elif(str(self.data)=='createToMapreduceFoder'):
      lustreSomething.createToMapreduceFoder()
...
class LustreSomething:  
  ...
  def getHostsInfo(self):
    ...
    r = requests.get('http://'+ambari_url+':8080/api/v1/clusters/'+cluster_name+'/services/LUSTREMGMTSERVICES/components/LustreClient', headers=headers, auth=('admin', 'admin'))
    j = json.loads(r.text)
    result =[]
  ...
  def get_lustrefs_config(self):
    ...
    r = requests.get('http://'+ambari_url+':8080/api/v1/clusters/'+cluster_name+'/services/LUSTREMGMTSERVICES/components/LustreMDSMgmtService', headers=headers, auth=('admin', 'admin'))
    j = json.loads(r.text)
  ...
  def mount(self):
    ...
    mdt_fsname = lustrefs_config["mdt_fsname"];
  ...
  def createToMapreduceFoder(self):
