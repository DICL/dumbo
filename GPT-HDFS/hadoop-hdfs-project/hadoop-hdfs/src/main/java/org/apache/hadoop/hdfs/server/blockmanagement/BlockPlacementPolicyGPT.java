...
public class BlockPlacementPolicyGPT extends BlockPlacementPolicy {
    ...
    private String datanodeInfo;
    private HashMap<Integer, List<DatanodeStorageInfo>> partitionMap;
    private int numReplicas;
    private boolean GPTInit;
    public FileSystem dfs;
    public String dfs_addr;
    ...
    protected BlockPlacementPolicyGPT() {
    }
    ...
    public void initialize(Configuration conf,  FSClusterStats stats,
        ...
        this.numNodes = conf.getInt("GPT.NumNodes", 10);
        this.maxPartitions = conf.getInt("GPT.MaxPartitions", 500);
        this.datanodeInfo = conf.get("GPT.Datanodes", "localhost");
        this.partitionMap = new HashMap<>();
        this.numReplicas = conf.getInt("dfs.replication", 3);
        this.GPTInit = false;
        try {
        ...
        this.dfs_addr = conf.get("fs.defaultFS");
    ...
    public DatanodeStorageInfo[] chooseTarget(String srcPath,
        ...
        if (!GPTInit) {
            try {
                GPTInitialize();
                LOG.info("GPT initialization complete! ready for partitionMap of size " + this.partitionMap.size());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }

        if(checkGPTPartitionedFile(srcPath) && numReplicas < numNodes) {

            // hdfs://10.150.20.24:8080/..../GPT-0-store_sales_text-0-00000.csv
            // hdfs://10.150.20.24:8080/..../GPT-0-store_sales_text-0-00000.snappy.parquet
            String[] tokens = srcPath.split("/");

            String partitionName = tokens[tokens.length - 1].split("\\.")[0];

            tokens = partitionName.split("-");
            String partitionNumber = "";
            // 00000._COPYING_
            if(tokens.length == 6) {
                // GPT-{Split}-{TableName}-{PartitionIndex}-{PartitionID}-{Overflow Index}
                partitionNumber = tokens[tokens.length-2];
            } else {
                // tokens.length == 5
                // GPT-{Split}-{TableName}-{PartitionIndex}-{PartitionID}
                partitionNumber = tokens[tokens.length-1];
            }
            int p = Math.abs(partitionNumber.hashCode() % this.numNodes);

            // LOG.info("partitionNumber: " + partitionNumber + ", hashVal: " + p);

            DatanodeStorageInfo[] results = new DatanodeStorageInfo[numReplicas];
            for(int i = 0 ; i < numReplicas ; i++) {
                results[i] = this.partitionMap.get(p).get(i);
            }
    ...
    DatanodeStorageInfo[] chooseTarget(String src,
        ...
        if (checkGPTPartitionedFile(src) && numReplicas < numNodes) {
    ...
    public boolean checkGPTPartitionedFile(String str) {
        if(str.contains("GPT") || str.contains("PREF"))
            return true;
        else
            return false;
    }

    public void GPTInitialize() throws IOException {

        boolean isPartitonMapExists = false;

        Path GPTPartitionMapPath = new Path(dfs_addr + "/PartitionMap");
        isPartitonMapExists = dfs.exists(GPTPartitionMapPath);

        if(!GPTInit) {

            List<DatanodeDescriptor> datanodes = new ArrayList<DatanodeDescriptor>(this.numNodes);

            String[] datanodeIPs = this.datanodeInfo.split(",");
            for(String dn : datanodeIPs) {
                datanodes.add(host2datanodeMap.getDatanodeByHost(dn));
            }

            List<DatanodeStorageInfo> dns = new ArrayList<>();
            for(int i = 0 ; i < this.numNodes ; i++) {
                DatanodeStorageInfo[] tmp = datanodes.get(i).getStorageInfos();
                dns.add(tmp[0].getDatanodeDescriptor().getStorageInfo(tmp[0].getStorageID()));
                LOG.info("Adding DatanodeDescriptor: " + tmp[0].getDatanodeDescriptor().getStorageInfo(tmp[0].getStorageID()));
            }

            if(isPartitonMapExists) {

                LOG.info("Loading GPT Partition Map!");
                BufferedReader br = null;
                br = new BufferedReader(new InputStreamReader(dfs.open(GPTPartitionMapPath)));

                String readLine = "";

                int partitionNum = -1;
                while((readLine = br.readLine()) != null) {
                    LOG.info("Partition Info: " + readLine);
                    String[] tokens = readLine.split("\t");
                    List<DatanodeStorageInfo> storageInfos = new ArrayList<>();
                    partitionNum = Integer.parseInt(tokens[0]);
                    for(int i = 1 ; i < tokens.length ; i++) {
                        int nodeIdx = Integer.parseInt(tokens[i]);
                        storageInfos.add(dns.get(nodeIdx));
                    }
                    this.partitionMap.put(partitionNum, storageInfos);
                }
                br.close();
...
/*
 def GPTInit(numPartition: Int, numReplica: Int, numNodes: Int) : Unit = {

      val writer = new PrintWriter(new FileWriter(new File("/data/GPT_PartitionMap")))

      (0 to numPartition-1).foreach{ p =>
        var partitionString = new StringBuffer()
        var nodeSet = new scala.collection.mutable.HashSet[Int]()
        partitionString.append(p).append("\t")
        (0 to numReplica -1).foreach{ r =>
          while (nodeSet.size < 3) {
            val nodeIdx = Random.nextInt(numNodes)
            nodeSet += nodeIdx
          }
        }
        var idx = 0
        nodeSet.foreach{ n =>
          if (idx < nodeSet.size - 1) {
            partitionString.append(n).append("\t")
          } else {
            partitionString.append(n)
          }
          idx += 1
        }
        partitionString.append("\n")
        writer.write(partitionString.toString)
      }
      writer.close()
    }
*/
                BufferedWriter bw = null;
                bw = new BufferedWriter(new OutputStreamWriter(dfs.create(GPTPartitionMapPath)));

                Random random = new Random();
                int nodeIdx = 0;
                int curNodeIdx = 0; // round-robin distribution! --> to avoid unbalanced partitioning

                for(int i = 0 ; i < this.maxPartitions ; i++) {
                    StringBuffer partitionString = new StringBuffer();
                    HashSet<Integer> nodeSet = new HashSet<>();
                    partitionString.append(i).append("\t");
                    for(int k = 0 ; k < this.numReplicas ; k++) {
                        while(nodeSet.size() < numReplicas) {
                            if(curNodeIdx % this.numNodes == 0) {
                                curNodeIdx = 0;
                            }
                            nodeIdx = curNodeIdx;
                            nodeSet.add(nodeIdx);
                    ...
                    for(int n : nodeSet) {
                        ...
                        storageInfos.add(dns.get(n));
                        idx += 1;
                    }
                    partitionString.append("\n");
                    this.partitionMap.put(i, storageInfos);
                    bw.write(partitionString.toString());
                }
                    bw.close();
            }
        }
