...
__device__ bool detecting_circle_type1(DATATYPE i, DATATYPE j,  DATATYPE r, DATATYPE margin)
{
	DATATYPE i_ = 2*i - 1;
	DATATYPE j_ = 2*j - 1; 
	...
	DATATYPE sum = i_*i_ + j_*j_;
	if ( sum < r*r && sum > (r-margin)*(r-margin))
		if ( !(i_ > 0 && j_ > 0)) rt = true;

	return rt;
}

__device__ bool detecting_circle_type2(DATATYPE i, DATATYPE j,  DATATYPE r, DATATYPE margin)
{
	DATATYPE i_ = 2*i - 1;
	DATATYPE j_ = 2*j - 1; 
...
__device__ DATATYPE getspeed(DATATYPE x, DATATYPE y, DATATYPE z, int speed_fun)
{
    switch(speed_fun)
    {
        case 0:
        case 1:
            return 1.0;
        case 2:
            return (6 + 5*sin(2*x*PI)*sin(2*y*PI)*sin(2*z*PI));
        case 3:
            //return (1 + .5*sin(20*x*PI)*sin(20*y*PI)*sin(20*z*PI));
            //if (x > 0.5 && y > 0.5 && z > 0.5 )
            //    return 0.0;
            return (1 + .5*sin(20*x*PI)*sin(20*y*PI)*sin(20*z*PI));
        case 4:
            if( -0.2825*x + 0.0975*y + 0.0900*z + 0.1631 > 0 && -0.0800*x -0.0200*y + 0.0200*z + 0.0220 > 0)
                return 0.25;
            else if( -0.2825*x + 0.0975*y + 0.0900*z + 0.1631 < 0 && -0.0800*x -0.0200*y + 0.0200*z + 0.0220 < 0)
                return 1.0;
            else
                return 0.5;
        case 5:
            if  (  detecting_circle_type2(x,y,0.3,0.1)
                || detecting_circle_type1(x,y,0.5,0.1)
                || detecting_circle_type2(x,y,0.7,0.1)
                || detecting_circle_type1(x,y,0.9,0.1) )
                return 0;
            else
                return 1.0;
        case 6:
            if  (  detecting_circle_type2(x,y,0.3,0.1)
                ...
                return 1.0;
        /* case 7: */
        /*     return max(1.0,200*exp(-1.0*1600*cloest_random1(x,y,z))); */
        /* case 8: */
        /*     return max(1.0,200*exp(-1.0*1600*cloest_random2(x,y,z))); */
        /* case 9: */
        /*     if( 0.4500*x + 0.7500*y - 0.7200 > 0 && 0.9500*x + 0.2000*y - 0.5225 > 0) */
        /*         return 0.25; */
        /*     else if( 0.4500*x + 0.7500*y - 0.7200 < 0 && 0.9500*x + 0.2000*y - 0.5225 < 0) */
        /*         return 1.0; */
        /*     else */
        /*         return 0.5; */
        default:
...
__device__ DATATYPE get_time_eikonal(DATATYPE a, DATATYPE b, DATATYPE c, DATATYPE s)
{
	DATATYPE ret, tmp;
...
__global__ void heat_kernel(float* updated,float* voxels, int bidx, int boff, int dimx, int dimy, int dimz, unsigned int* num_vert)
{
    ...
    if ( x >= 0 && y >= 0 && z >= 0 &&
         x < dimx && y < dimy && z < dimz+2){
   
        int index = offset_3d({x,y,z},Vec3i(dimx));
 
        updated[index] = voxels[index];
...
__global__ void mc_kernel(Vertex* vertices,float* voxels, int bidx, int boff, int dimx, int dimy, int dimz, unsigned int* num_vert,float* isolist, int start, int end, int N, float* color_table)
{
    int x = threadIdx.x + blockDim.x * blockIdx.x;
    int y = threadIdx.y + blockDim.y * blockIdx.y;
    int z = threadIdx.z + blockDim.z * blockIdx.z;
    ...
    if ( x >= 0 && y >=0  && z >= 0 &&
        ...
        { 
            z = threadIdx.z + blockDim.z * blockIdx.z + 1;

            for (int i = 0 ; i  < 8; i++)
                vs[i] =INF;

            float isovalue = isolist[j];
        ...
        //              int edge_indices[12];
            ...
            #if 0
            {
                printf("[%d,%d,%d] -> %f,%f,%f,%f,%f,%f,%f,%f [%f]\n",
                       ...
                       vs[0],vs[1],vs[2],vs[3],vs[4],vs[5],vs[6],vs[7],isovalue);
            }
            #endif
            //num_vert+= n_indices;
...
*/
__global__ void eikonal_kernel(float* sol_out,float* sol_in, int bidx, int boff, int dimx, int dimy, int dimz, unsigned int* num_vert, int block_num, int type)
{
         ...
         x < dimx && y < dimy && z < dimz)
	{
		...
		DATATYPE a,b,c,oldT,newT;
        int halo = 1;
    
        DATATYPE gx = 1.0*x/dimx;
        DATATYPE gy = 1.0*y/dimy;
        DATATYPE gz = 1.0*(bidx*boff+z)/dimz;
        
        F = getspeed(gx,gy,gz,type);        
 
        z += halo;       
 
        DATATYPE vs[6] = {INF,INF,INF,INF,INF,INF};

        newT = oldT =  sol_in[offset_3d({x,y,z},   Vec3i((dimx)))];
        
        if (x > 0) 
            vs[0] = sol_in[offset_3d({x-1, y,z},   Vec3i((dimx)))];
        if (x < dimx-1)       
            vs[1] = sol_in[offset_3d({x+1, y,z},   Vec3i((dimx)))];
        if (y > 0) 
            vs[2] = sol_in[offset_3d({x, y-1,z},   Vec3i((dimx)))];
        if (y < dimy-1) 
            vs[3] = sol_in[offset_3d({x, y+1,z},   Vec3i((dimx)))];
        if (bidx > 0) 
            vs[4] = sol_in[offset_3d({x, y,z-1},   Vec3i((dimx)))];
        if (bidx < block_num-1) 
            vs[5] = sol_in[offset_3d({x, y,z+1},   Vec3i((dimx)))];
        ...
        DATATYPE tmp = (DATATYPE) get_time_eikonal(a, b, c, F);

        newT = min(tmp,oldT);
        sol_out[offset_3d({x,y,z},   Vec3i((dimx)))] = newT;
        //sol_out[offset_3d({x,y,z},   Vec3i((dimx)))] = bidx;
	}
