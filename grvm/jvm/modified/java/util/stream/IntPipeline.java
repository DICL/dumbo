...
//GRVM:begin
    private static final boolean EnableGPUOffloading = System.getenv("DGRVM") == null;
//GRVM:end
    @Override
    public void forEach(IntConsumer action) {
//GRVM:begin
//      evaluate(ForEachOps.makeInt(action, false));
    	if (EnableGPUOffloading) {
    		if (getSpliterator() instanceof Streams.RangeIntSpliterator) {
    			Streams.RangeIntSpliterator rit = (Streams.RangeIntSpliterator)getSpliterator();
    			int from = rit.from;
    			int last = rit.upTo;
    			try {
						Class<?> cls = Class.forName("org.grvm.dispatch.Dispatcher");
						java.lang.reflect.Method m = cls.getMethod("offloadIntStream", int.class, int.class, IntConsumer.class);
						m.invoke(null, from, last, action);
					} catch (Throwable e) {
