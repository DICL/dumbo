...
private[spark] class TaskSetManager(
  ...
  /*
  var isGPTTask: Boolean = false
  def setGPTTaskScheduling() : Unit = { isGPTTask = true }
  def deSetGPTTaskScheduling() : Unit = { isGPTTask = false }
  def getGPTTaskScheduling() : Boolean = { isGPTTask }
  var GPTHost2PartitionStack : scala.collection.mutable.HashMap[String, scala.collection.mutable.Stack[Int]] = null
  def setGPTHost2PartitionStack(stack: scala.collection.mutable.HashMap[String, scala.collection.mutable.Stack[Int]]) : Unit = {
    GPTHost2PartitionStack = stack
  }
  def getGPTHost2PartitionStack : scala.collection.mutable.HashMap[String, scala.collection.mutable.Stack[Int]] = {
    GPTHost2PartitionStack
  }

  var sanityCheck = 0.0
  def setSanityCheck(num: Double): Unit = {
    this.sanityCheck = num
  }
  def getSanityCheckValue() : Double = {
    sanityCheck
  }
  */
  ...
  /*
  // partition ID --> {host IDs}
  var GPTPartition2HostMap = sched.getGPTPartition2HostMap

  val GPTConfName = "GPTTask_" + numTasks + "_" + taskSet.id
  val GPTConf = sched.sc.conf.getOption(GPTConfName)
  */
  ...
  /*
  private def dequeueTaskFromListGPT(
                                   execId: String,
                                   host: String,
                                   list: ArrayBuffer[Int],
                                   GPTHost2PartitionStack: mutable.HashMap[String, mutable.Stack[Int]],
                                   remainingTasks: collection.mutable.ArrayBuffer[Int],
                                   condString: String): Option[Int] = {

    val partitionStack = GPTHost2PartitionStack(host)
    var index = -1
    if (!isTaskBlacklistedOnExecOrNode(index, execId, host)) {
      if (partitionStack.isEmpty) {
        None
      } else {
        index = partitionStack.pop()
        //logInfo(s"[" + getSanityCheckValue() + "] try to schedule! " + host + " -> " + partitionStack)
      }
    } else {
        if (copiesRunning(index) == 0 && !successful(index)) {
          logDebug(s"[ " + condString + "], execID: " + execId + ", host: " + host + ", partitionIndex: " + index)
          return Some(index)
        }
      }
    None
  ...
  */
    ...
    : Option[TaskDescription] =
  {
    ...
    if (!isZombie && !offerBlacklisted) {
      ...
      logDebug(s"resourceOffer: [execID: " + execId + "], [host: " + host + "]," +
        " [maxLocality: " + maxLocality + "]")

      /*
      if (isGPTTask) {
        allowedLocality = TaskLocality.NODE_LOCAL
      }
      */
  ...
  /*
  private def dequeueTaskGPT(execId: String, host: String, maxLocality: TaskLocality.Value,
                             remainingTasks: collection.mutable.ArrayBuffer[Int])
  : Option[(Int, TaskLocality.Value, Boolean)] =
  {

    for (index <- dequeueTaskFromListGPT(execId, host, getPendingTasksForExecutor(execId), GPTHost2PartitionStack, remainingTasks, "TaskLocality.PROCESS_LOCAL")) {
      return Some((index, TaskLocality.PROCESS_LOCAL, false))
    }

    //dequeueTask(execId: String, host: String, maxLocality: TaskLocality.Value)


    if (TaskLocality.isAllowed(maxLocality, TaskLocality.NODE_LOCAL)) {
      for (index <- dequeueTaskFromListGPT(execId, host, getPendingTasksForHost(host), GPTHost2PartitionStack, remainingTasks, "TaskLocality.NODE_LOCAL")) {
        return Some((index, TaskLocality.NODE_LOCAL, false))
      }
    }
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.NO_PREF)) {
      // Look for noPref tasks after NODE_LOCAL for minimize cross-rack traffic
      for (index <- dequeueTaskFromListGPT(execId, host, pendingTasksWithNoPrefs, GPTHost2PartitionStack, remainingTasks, "TaskLocality.NO_PREF")) {
        return Some((index, TaskLocality.PROCESS_LOCAL, false))
      }
    }
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.RACK_LOCAL)) {
      for {
        rack <- sched.getRackForHost(host)
        index <- dequeueTaskFromListGPT(execId, host, getPendingTasksForRack(rack), GPTHost2PartitionStack, remainingTasks, "TaskLocality.RACK_LOCAL")
      } {
        return Some((index, TaskLocality.RACK_LOCAL, false))
      }
    }
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.ANY)) {
      for (index <- dequeueTaskFromListGPT(execId, host, allPendingTasks, GPTHost2PartitionStack, remainingTasks, "TaskLocality.ANY")) {
        return Some((index, TaskLocality.ANY, false))
      }
    }
  ...
  */
  ...
  : Option[TaskDescription] =
  {
    ...
    if (!isZombie && !offerBlacklisted) {
      ...
      logDebug(s"resourceOfferForGPT: [execID: " + execId + "], [host: " + host + "]," +
        " [maxLocality: " + maxLocality + "]")

      val remainingTasks = new collection.mutable.ArrayBuffer[Int]()
      (0 to numTasks-1).foreach(c => remainingTasks += c)

      //dequeueTaskGPT(execId, host, allowedLocality, remainingTasks).map { case ((index, taskLocality, speculative)) =>
      dequeueTask(execId, host, TaskLocality.NODE_LOCAL).map { case ((index, taskLocality, speculative)) =>
        val task = tasks(index)
        val taskId = sched.newTaskId()
        copiesRunning(index) += 1
        val attemptNum = taskAttempts(index).size
        ...
        val taskName = s"task ${info.id} in stage ${taskSet.id}"
        logDebug(s"[GPT] Starting $taskName (TID $taskId, $host, executor ${info.executorId}, " +
