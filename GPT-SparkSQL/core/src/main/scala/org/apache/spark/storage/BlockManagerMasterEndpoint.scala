  ...
  def updateBlockInfo(
    ...
    if (storageLevel.isValid) {
      ...
      if (storageLevel.useMemory) {
        ...
        if (blockExists) {
          logDebug(s"Updated $blockId in memory on ${blockManagerId.hostPort}" +
            s" (current size: ${Utils.bytesToString(memSize)}," +
            s" original size: ${Utils.bytesToString(originalMemSize)}," +
            s" free: ${Utils.bytesToString(_remainingMem)})")
        } else {
          logDebug(s"Added $blockId in memory on ${blockManagerId.hostPort}" +
      ...
      if (storageLevel.useDisk) {
        ...
        _blocks.put(blockId, blockStatus)
        if (blockExists) {
          logDebug(s"Updated $blockId on disk on ${blockManagerId.hostPort}" +
            s" (current size: ${Utils.bytesToString(diskSize)}," +
            s" original size: ${Utils.bytesToString(originalDiskSize)})")
        } else {
          logDebug(s"Added $blockId on disk on ${blockManagerId.hostPort}" +
      ...
      _cachedBlocks -= blockId
      if (originalLevel.useMemory) {
        logDebug(s"Removed $blockId on ${blockManagerId.hostPort} in memory" +
          s" (size: ${Utils.bytesToString(originalMemSize)}," +
          s" free: ${Utils.bytesToString(_remainingMem)})")
      }
      if (originalLevel.useDisk) {
        logDebug(s"Removed $blockId on ${blockManagerId.hostPort} on disk" +
