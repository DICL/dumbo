...
class DAGScheduler(
  ...
  private def getOrCreateParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = {
    logDebug(s"getOrCreateParentStages | RDD.id: " + rdd.id + ", RDD partition size: " + rdd.partitions.length + ", firstJobID: " + firstJobId)
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                ...
                val stageRDDScopeID = stage.rdd.scope.get.id
                val stageRDDScopeName = stage.rdd.scope.get.name
                val stageString = stage.toString
                logDebug(s"getMissingParentStages | RDD.id: " + rdd.id + ", RDD partition size: " + rdd.partitions.length + ", firstJobID: " + stage.firstJobId)
                logDebug(s"[submitStage] stageString: " + stageString + ", stageRDDScopeID: " + stageRDDScopeID + ", stageRDDScopeName: " + stageRDDScopeName)
  ...
  private[scheduler] def handleMapStageSubmitted(jobId: Int,
    ...
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      logDebug(s"handleMapStageSubmitted | jobId: " + jobId)
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          val stageRDDScopeName = stage.rdd.scope.get.name
          val stageString = stage.toString
          val jobIDString = jobId

          logDebug(s"[submitStage]: jobIDString: " + jobIDString + ", stageString: " + stageString + ", stageRDDScopeID: " + stageRDDScopeID + ", stageRDDScopeName: " + stageRDDScopeName)
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    if (tasks.size > 0) {
      ...
      logDebug(s"[submitMissingTasks] stage.RDD.id: " + stage.rdd.id + ", jobIDString: " + jobIDString + ", stageString: " + stageString + ", stageRDDScopeID: " + stageRDDScopeID + ", stageRDDScopeName: " + stageRDDScopeName)
      val taskSet = new TaskSet(tasks.toArray, stage.id, stage.latestInfo.attemptId, jobId, properties)
        ...
        for (dep <- rdd.dependencies) {
          dep match {
            ...
            case shufDep: ShuffleDependency[_, _, _] =>
              ...
              val stageRDDScopeName = stage.rdd.scope.get.name
              val stageString = stage.toString
              logDebug(s"stageDependsOn | RDD.id: " + rdd.id + ", RDD partition size: " + rdd.partitions.length + ", firstJobID: " + stage.firstJobId)
              logDebug(s"[submitStage] stageString: " + stageString + ", stageRDDScopeID: " + stageRDDScopeID + ", stageRDDScopeName: " + stageRDDScopeName)
