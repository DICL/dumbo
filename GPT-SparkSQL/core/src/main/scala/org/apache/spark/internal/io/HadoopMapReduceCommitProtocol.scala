...
class HadoopMapReduceCommitProtocol(jobId: String, path: String)
  ...
  private def getFilename(taskContext: TaskAttemptContext, ext: String): String = {
    ...
    if (taskContext.getConfiguration.getBoolean("GPT", false)) {
      f"GPT-$split%05d"
    } else {
  ...
  override def commitJob(jobContext: JobContext, taskCommits: Seq[TaskCommitMessage]): Unit = {
    ...
    logInfo(s"[HadoopMapReduceCommitProtocol] comitting job context...")
    committer.commitJob(jobContext)
    logInfo(s"[HadoopMapReduceCommitProtocol] moving files...")
