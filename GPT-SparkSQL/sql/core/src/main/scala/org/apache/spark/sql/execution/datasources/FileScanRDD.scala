...
class FileScanRDD(
  ...
  val checkSanity = math.random
  var isGPTScanTask = false

  logDebug(s"[" + checkSanity + "] Call FileScanRDD: " + readFunction.toString())
  var partitionNum = 0
  filePartitions.foreach{
    fp =>
      fp.files.foreach{
        c =>
          logDebug(s"[" + checkSanity + "], [FilePartition Idx: " + fp.index + "], Compute files: " + c.filePath)
          if (c.filePath.contains("GPT")) {
            isGPTScanTask = true
          }
      }
      partitionNum += 1
