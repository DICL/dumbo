...
case class SortMergeJoinExec(
  ...
  def findScanOP_v2(p: SparkPlan, joinCol: String, scanOP: scala.collection.mutable.ArrayBuffer[FileSourceScanExec]) : Unit = p match {
    // p : is logical plan or any kind of query operator

    case _ if !p.isInstanceOf[FileSourceScanExec] => {
      val plan = p
      ...
      childSize match {
        case 1 => {
          ...
          findScanOP_v2(plan.children(0), joinCol, scanOP)
        }
        case 2 => {
          //logDebug(s"curPlan Child-0 OP: " + plan.children(0).nodeName)
          //logDebug(s"curPlan Child-1 OP: " + plan.children(1).nodeName)
          findScanOP_v2(plan.children(0), joinCol, scanOP)
          findScanOP_v2(plan.children(1), joinCol, scanOP)
        }
        case 0 => {
          Unit
        }
    ...
    case _ if p.isInstanceOf[FileSourceScanExec] => {
      val candidateRel = p.asInstanceOf[FileSourceScanExec]
  ...
  override def outputPartitioning: Partitioning = joinType match {
    case _: InnerLike => {
      ...
      logDebug("Left.numPartition = " + left.outputPartitioning.numPartitions + ", Right.numPartition: " + right.outputPartitioning.numPartitions)

      val leftScanOP = findScanOP(left)
      val rightScanOP = findScanOP(right)

      if ( ((leftScanOP.isInstanceOf[FileSourceScanExec])&& (rightScanOP.isInstanceOf[FileSourceScanExec])) &&
        ( (isGPTPartitiondTable(leftScanOP.asInstanceOf[FileSourceScanExec])) ||
        (isGPTPartitiondTable(rightScanOP.asInstanceOf[FileSourceScanExec])) ) &&
        (right.outputPartitioning == UnknownPartitioning(0)) ) {

        val leftOP = leftScanOP.asInstanceOf[FileSourceScanExec]
        val rightOP = rightScanOP.asInstanceOf[FileSourceScanExec]



        // case 1: repartition join
        // SortMergeJoinExec: PartitioningCollection| [left: hashpartitioning(ss_item_sk#15, 240)], [right: hashpartitioning(i_item_sk#203, 240)]
        // SortMergeJoinExec: Seq(left.outputPartitioning, right.outputPartitioning).map(_.numPartitions): List(240, 240)
        // SortMergeJoinExec: Seq(left.outputPartitioning, right.outputPartitioning).map(_.numPartitions).distinct: List(240)

        // case 2: skip such plan!
        // SortMergeJoinExec: PartitioningCollection| [left: (hashpartitioning(ss_item_sk#15, 240) or hashpartitioning(i_item_sk#203, 240))], [right: UnknownPartitioning(0)]
        // SortMergeJoinExec: Seq(left.outputPartitioning, right.outputPartitioning).map(_.numPartitions): List(240, 0)
        // SortMergeJoinExec: Seq(left.outputPartitioning, right.outputPartitioning).map(_.numPartitions).distinct: List(240, 0)
        PartitioningCollection(Seq(left.outputPartitioning, left.outputPartitioning))
  ...
  override def requiredChildDistribution: Seq[Distribution] = {
    ...
    val leftStorage = scala.collection.mutable.ArrayBuffer[FileSourceScanExec]()
    val rightStorage = scala.collection.mutable.ArrayBuffer[FileSourceScanExec]()
    findScanOP_v2(left, leftKeys(0).references.toSeq(0).name, leftStorage)
    findScanOP_v2(right, rightKeys(0).references.toSeq(0).name, rightStorage)


    var leftDistribution: ClusteredDistribution = null
    var rightDistribution: ClusteredDistribution = null

    logDebug(s"[SMJ-Exec] leftKeys: " + leftKeys.mkString(", ") + ", rightKeys: " + rightKeys.mkString(", "))

    if(!leftStorage.isEmpty && !rightStorage.isEmpty) {

      val leftScanOP = leftStorage(0)
      val rightScanOP = rightStorage(0)

      if (isGPTPartitiondTable(leftScanOP)) {
        var leftGPTJoinCols = leftScanOP.getGPTJoinCols(leftScanOP.relation.bucketSpec.get)
        logDebug(s"[SMJ-Exec] leftGPTJoinCols: " + leftGPTJoinCols.mkString(", ") + " (tableName: " + leftScanOP.tblName + ")")
        var leftClusteredCols: scala.collection.mutable.ArrayBuffer[Expression] = new scala.collection.mutable.ArrayBuffer[Expression]()
        leftKeys.foreach { c =>
          c.references.foreach { k =>
            if (leftGPTJoinCols.contains(k.name)) {
              leftClusteredCols += c
            }
          }
        }

        if (!leftClusteredCols.isEmpty)
          leftDistribution = ClusteredDistribution(leftClusteredCols.toSeq)
        logDebug(s"[SMJ-Exec] leftClusteredCols: " + leftClusteredCols.mkString(", ") + " (tableName: " + leftScanOP.tblName + ")")

      }

      if (isGPTPartitiondTable(rightScanOP)) {
        var rightGPTJoinCols = rightScanOP.getGPTJoinCols(rightScanOP.relation.bucketSpec.get)
        logDebug(s"[SMJ-Exec] rightGPTJoinCols: " + rightGPTJoinCols.mkString(", ") + " (tableName: " + rightScanOP.tblName + ")")

        var rightClusteredCols: scala.collection.mutable.ArrayBuffer[Expression] = new scala.collection.mutable.ArrayBuffer[Expression]()
        rightKeys.foreach { c =>
          c.references.foreach { k =>
            if (rightGPTJoinCols.contains(k.name)) {
              rightClusteredCols += c
            }
          }
        }

        if (!rightClusteredCols.isEmpty)
          rightDistribution = ClusteredDistribution(rightClusteredCols.toSeq)
        logDebug(s"[SMJ-Exec] rightClusteredCols: " + rightClusteredCols.mkString(", ") + " (tableName: " + rightScanOP.tblName + ")")

      }
    }

    if(leftDistribution == null) {
      leftDistribution = ClusteredDistribution(leftKeys)
      logDebug(s"[SMJ-Exec] leftClusteredCols (null handling): " + leftKeys.mkString(", "))

    }
    if(rightDistribution == null) {
      rightDistribution = ClusteredDistribution(rightKeys)
      logDebug(s"[SMJ-Exec] rightClusteredCols (null handling): " + rightKeys.mkString(", "))

    }

    leftDistribution :: rightDistribution :: Nil
  }
