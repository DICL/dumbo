...
case class DataSource(
  ...
  private def writeInFileFormat(format: FileFormat, mode: SaveMode, data: DataFrame): Unit = {
    ...
    val ParallelMerge = if (sparkSession.sparkContext.conf.get("spark.GPT.ParallelMerge") == "true") true else false
    val isGPTMergeTask =  sparkSession.sessionState.conf.getConfString("GPTMergerJob")

    if(ParallelMerge && isGPTMergeTask == "true") {
      logInfo(s"Execute Parallel GPT Merger")

      val outputFileName = outputPath.getName
      val mergePlan = GPTMergerJob(outputPath = outputPath, options = options).run(sparkSession)
    }
  }
