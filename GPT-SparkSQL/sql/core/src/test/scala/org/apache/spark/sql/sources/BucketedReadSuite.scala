...
//      }
  ...
  test("FIND THE SECRET!") {
    val bucketSpec1 = Some(BucketSpec(8, Seq("i", "j"), Nil))
    val bucketSpec2 = Some(BucketSpec(8, Seq("j"), Nil))

    val bucketedTableTestSpecLeft = BucketedTableTestSpec(bucketSpec1, expectedShuffle = false)
    val bucketedTableTestSpecRight = BucketedTableTestSpec(bucketSpec2, expectedShuffle = false)
    testBucketing(
