  ...
  private def reorder(plan: LogicalPlan, output: Seq[Attribute]): LogicalPlan = {
    ...
    val result =
      ...
      if (items.size > 2 && items.size <= conf.joinReorderDPThreshold && conditions.nonEmpty &&
        ...
        //logDebug(s"[CostBasedJoinReorder] extractInnerJoins for given plan: " + plan.simpleString)
        //items.foreach(c => logDebug(s"\tjoinPlan: " + c.simpleString) )
        //conditions.foreach(c => logDebug(s"\tjoinCond: " + c.verboseStringWithSuffix) )

        JoinReorderDP.search(conf, items, conditions, output)

      } else {
        //logDebug(s"[CostBasedJoinReorder] return the same plan without join reordering")
        plan
      }
    //logDebug(s"[CostBasedJoinReorder] return the reordered join!")
...
object JoinReorderDP extends PredicateHelper with Logging {
  ...
  private def searchLevel(
    ...
    while (k <= lev - k) {
      //logDebug(s"[JoinReorder.searchLevel] curretLevel: " + k + ", totLevel: " + existingLevels + ", # cadidiate plans: " + existingLevels(k).values.toSeq.size)
      val oneSideCandidates = existingLevels(k).values.toSeq
      for (i <- oneSideCandidates.indices) {

        val oneSidePlan = oneSideCandidates(i)
        //logDebug(s"[JoinReorder.searchLevel]\t" + i + "-th plan:" + oneSidePlan.plan.simpleString)
        //oneSidePlan.itemIds.foreach(c => logDebug(s"\t\titemID: " + c))
        //oneSidePlan.joinConds.foreach(c => logDebug(s"\t\tjoinCond: " + c))
        //logDebug(s"\t\tcurPlanCardinality: " + oneSidePlan.planCost.card.toDouble + ", size: " + oneSidePlan.planCost.size.toDouble)

        val otherSideCandidates = if (k == lev - k) {
          // Both sides of a join are at the same level, no need to repeat for previous ones.
          //logDebug(s"[JoinReorder.searchLevel] drop current cadidates at the same level")
        ...
        otherSideCandidates.foreach { otherSidePlan =>
          //logDebug(s"[JoinReorder.searchLevel @ enumeration]\t" + i + "-th plan:" + oneSidePlan.plan.simpleString)
          //oneSidePlan.itemIds.foreach(c => logDebug(s"\t\titemID: " + c))
          //oneSidePlan.joinConds.foreach(c => logDebug(s"\t\tjoinCond: " + c))
          //logDebug(s"\t\tcurPlanCardinality: " + oneSidePlan.planCost.card.toDouble + ", size: " + oneSidePlan.planCost.size.toDouble)
    ...
    nextLevel.foreach{ c =>
      //logDebug(s"[JoinReorder.searchLevel] reorderd plan!")
      nextLevel.foreach{ c =>
        //c._1.foreach(c => logDebug(s"\tcurrent itemIDs: " + c))
        //c._2.joinConds.foreach(c => logDebug(s"\tjoinConds: " + c.verboseStringWithSuffix))
        //c._2.plan.foreach(c => logDebug(s"\tPlan: " + c.simpleString))
      }
    }
    nextLevel.toMap
  }
 ...
 */
object JoinReorderDPFilters extends PredicateHelper with Logging{
  /**
  ...
  def buildJoinGraphInfo(
    ...
    if (conf.joinReorderDPStarFilter) {
      ...
      if (starJoin.nonEmpty && nonStarJoin.nonEmpty) {
        ...
        //logDebug(s"[buildJoinGraphInfo] found starJoin!")
        //starJoin.foreach(c => logDebug(s"\t" + c.verboseStringWithSuffix))
        //logDebug(s"[buildJoinGraphInfo] found nonStarJoin!")
        //nonStarJoin.foreach(c => logDebug(s"\t" + c.verboseStringWithSuffix))
