  ...
  def unapply(plan: LogicalPlan): Option[ReturnType] = plan match {
    case join @ Join(left, right, joinType, condition) =>
      ...
      if (joinKeys.nonEmpty) {
        ...
        var leftJoinCols: scala.collection.mutable.ListBuffer[String]
        = new scala.collection.mutable.ListBuffer[String]()
        var rightJoinCols: scala.collection.mutable.ListBuffer[String]
        = new scala.collection.mutable.ListBuffer[String]()

        leftKeys.foreach(p => leftJoinCols += p.references.mkString(", ").split("#")(0))
        rightKeys.foreach(p => rightJoinCols += p.references.mkString(", ").split("#")(0))

        val leftJoinColSet = leftJoinCols.toSet
        val rightJoinColSet = rightJoinCols.toSet

        leftJoinColSet.foreach(p => if (!(left.getJoinCols.contains(p))) left.getJoinCols += p)
        rightJoinColSet.foreach(p => if (!(right.getJoinCols.contains(p))) right.getJoinCols += p)

        left.getJoinCols.foreach(p => logDebug(s"left JoinCols: " + p))
        right.getJoinCols.foreach(p => logDebug(s"right JoinCols: " + p))
