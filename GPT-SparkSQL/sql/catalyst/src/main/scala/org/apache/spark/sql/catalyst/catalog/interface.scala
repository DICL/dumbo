...
case class BucketSpec(
  ...
  var isGPTPartitioned = false

  def setGPTPartitioned() : Unit = { this.isGPTPartitioned = true }
  def getISGPTPartitioned() : Boolean = { isGPTPartitioned }
  ...
  override def toString: String = {
    ...
    val GPTString = if (isGPTPartitioned) {
      s", GPT Partitionined table"
    } else {
      ""
    }
    s"$numBuckets buckets, $bucketString$sortString$GPTString"
  }

  def toLinkedHashMap: mutable.LinkedHashMap[String, String] = {
    val GPTString = if (isGPTPartitioned) s"YES" else s"NO"
...
case class CatalogTable(
  ...
  def toLinkedHashMap: mutable.LinkedHashMap[String, String] = {
    ...
    if (bucketSpec.isDefined) {
      if (bucketSpec.get.isGPTPartitioned) {
        map.put("GPT", "true")
      } else {
        map.put("GPT", "false")
      }
