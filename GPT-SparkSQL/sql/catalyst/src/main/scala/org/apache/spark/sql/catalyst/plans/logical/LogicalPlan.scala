...
abstract class LogicalPlan extends QueryPlan[LogicalPlan] with Logging {
  ...
  /**
    ...
    */
  var joinTypes: scala.collection.mutable.HashMap[String, scala.collection.mutable.HashSet[String]] = new mutable.HashMap[String,scala.collection.mutable.HashSet[String]]()
  var joinCols: scala.collection.mutable.ArrayBuffer[String] = new scala.collection.mutable.ArrayBuffer[String]()
  def addJoinCols(col: String) = {joinCols += col}
  def getJoinCols: scala.collection.mutable.ArrayBuffer[String] = {
    joinCols
  }
  def addJoinType(joinCol: String, joinType: String) = {

    if(joinTypes.contains(joinCol)) {
      joinTypes.get(joinCol).get.add(joinType)
    } else {
      val set = new scala.collection.mutable.HashSet[String]()
      set.add(joinType)
      joinTypes += Tuple2(joinCol, set)
    }
  }

  def getJoinType(joinCol: String) : String = {
    if(joinTypes.nonEmpty) {
      joinTypes.foreach(c => if (c._1 == joinCol) c._2.mkString(","))
    }
    "NONE"
  }

  def getJoinTypes : scala.collection.mutable.HashMap[String, scala.collection.mutable.HashSet[String]] = {
    this.joinTypes
  }

  var GPTJoinType: String = "None"
  def setGPTJoinType(joinType: String) : Unit = {
    this.GPTJoinType = joinType
  }
  def getGPTJoinType() : String = {
    this.GPTJoinType
  }

  var isCyclicJoin: Boolean = false
