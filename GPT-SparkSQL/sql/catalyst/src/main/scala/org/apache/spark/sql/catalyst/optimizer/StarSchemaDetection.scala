 ...
 */
case class StarSchemaDetection(conf: SQLConf) extends PredicateHelper with Logging {
  ...
  def findStarJoins(
    ...
    //logDebug(s"[findStarJoins] searching star join!")

    if (input.size < 2) {
      //logDebug(s"[findStarJoins] return emptyStarJoinPlan due to input plan's size < 2")
      emptyStarJoinPlan
    } else {
      ...
      if (!foundEligibleJoin) {
        ...
        //logDebug(s"[findStarJoins] return emptyStarJoinPlan due to no eligibleJoin")
        emptyStarJoinPlan
      } else {
        ...
        //sortedFactTables.foreach( c => logDebug(s"\t[findStarJoins] found fact table: " + c.plan.simpleString))

        sortedFactTables match {
          case Nil =>
            //logDebug(s"[findStarJoins] return emptyStarJoinPlan due to no fact table exists")
            emptyStarJoinPlan
          case table1 :: table2 :: _
            ...
            //logDebug(s"[findStarJoins] return emptyStarJoinPlan due to table1.size! (table1.size: " + table1.size.get.toDouble + ", conf Val: " + conf.starSchemaFTRatio * table1.size.get.toDouble + ")")
            emptyStarJoinPlan
          case TableAccessCardinality(factTable, _) :: rest =>
            ...
            //logDebug(s"[findStarJoins] fact joins and their conditions")
            //allFactJoins.foreach(c => logDebug(s"\tfound fact join: " + c.simpleString))
            //allFactJoinCond.foreach(c => logDebug(s"\tfound fact join condition: " + c.simpleString))
            ...
            if (!areStatsAvailable) {
              //logDebug(s"[findStarJoins] return emptyStarJoinPlan due to no stats are available")
              emptyStarJoinPlan
            } else {
              ...
              if (eligibleDimPlans.isEmpty || eligibleDimPlans.size < 2) {
                ...
                //logDebug(s"[findStarJoins] return emptyStarJoinPlan due to eligibleDimPlans.isEmpty || eligibleDimPlans.size < 2")
                emptyStarJoinPlan
              } else {
                //logDebug(s"[findStarJoins] We return the result!")
                //eligibleDimPlans.foreach(c => logDebug(s"\t[findStarJoins] elibigleDimPlan: ") + c.verboseStringWithSuffix)
                //factTable.foreach(c => logDebug(s"\t[findStarJoins] factTable: ") + c.verboseStringWithSuffix)
  ...
  def reorderStarJoins(
    ...
    if (starPlan.isEmpty) {
      //logDebug(s"[reorderStarJoins] return EmptyStarJoinPlan due to no star join plan")
      emptyStarJoinPlan
    } else {
      val (factTable, dimTables) = (starPlan.head, starPlan.tail)
      //logDebug(s"[reorderStarJoins] factTablePlan (starPlan.head): " + factTable.verboseStringWithSuffix)
      //dimTables.foreach(c => logDebug(s"[reorderStarJoins] dimTablePlan (starPlan.tail): " + c.verboseStringWithSuffix))
      ...
      if (isSelectiveStarJoin(dimTables, conditions)) {
        //logDebug(s"[reorderStarJoins] selective PK-FK join with condition")
        //dimTables.foreach(c => logDebug(s"\tdimTable:" + c.verboseStringWithSuffix))
        //conditions.foreach(c => logDebug(s"\tselective cond: " + c.verboseStringWithSuffix))
        ...
        //logDebug(s"[reorderStarJoins] reordered starplan")
        val reorderStarPlan = factTable +: reorderDimTables
        //reorderStarPlan.foreach( c=> logDebug(s"\t" + c.verboseStringWithSuffix))
        reorderStarPlan.map(plan => (plan, Inner))
      } else {
        //logDebug(s"[reorderStarJoins] return EmptyStarJoinPlan due to no selective star join")
        emptyStarJoinPlan
      }
    }
  }
