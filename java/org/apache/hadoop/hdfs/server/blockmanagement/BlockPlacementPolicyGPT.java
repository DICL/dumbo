...
public class BlockPlacementPolicyGPT extends BlockPlacementPolicy {
    ...
    public DatanodeStorageInfo[] chooseTarget(String srcPath,
        ...
        if(checkGPTPartitionedFile(srcPath)) {
            ...
            // 00000._COPYING_
            String partitionNumber = tokens[tokens.length-1];
            int p = Math.abs(partitionNumber.hashCode() % this.numNodes);

            LOG.info("partitionNumber: " + partitionNumber + ", hashVal: " + p);
            DatanodeStorageInfo[] results = new DatanodeStorageInfo[this.partitionMap.get(p).size()];
            this.partitionMap.get(p).toArray(results);
    ...
    DatanodeStorageInfo[] chooseTarget(String src,
        ...
        if (checkGPTPartitionedFile(src)) {
...
*/
                ...
                for(int i = 0 ; i < this.maxPartitions ; i++) {
                    ...
                    for(int k = 0 ; k < this.numReplicas ; k++) {
                        while(nodeSet.size() < 3) {
                            nodeIdx = random.nextInt(numNodes);
                            nodeSet.add(nodeIdx);
                        }
                    }
                    int idx = 0;
