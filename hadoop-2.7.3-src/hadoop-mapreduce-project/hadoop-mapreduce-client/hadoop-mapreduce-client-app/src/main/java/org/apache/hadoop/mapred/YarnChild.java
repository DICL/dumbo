...
class YarnChild {
	...
	public static void main(String[] args) throws Throwable {
		...
		String useGPU = System.getProperty("useGPU", "false");
		if (useGPU.startsWith("true"))
			bUseGPU = true;
		else
			bUseGPU = false;
		
		String mustGPU = System.getProperty("mustGPU", "false");
		if (mustGPU.startsWith("true"))
			bMustGPU = true;
		else
			bMustGPU = false;
	 ...
	 * the user credentials of the user running the Map / Reduce Task
	 * 
	 * @param task
	 *            The Map / Reduce task to set the Encrypted Spill information
	 *            in
	 * @throws Exception
	 */
	public static void setEncryptedSpillKeyIfRequired(Task task)
			throws Exception {
		if ((task != null) && (task.getEncryptedSpillKey() != null)
				&& (task.getEncryptedSpillKey().length > 1)) {
	...
	private static void configureLocalDirs(Task task, JobConf job)
			throws IOException {
		String[] localSysDirs = StringUtils.getTrimmedStrings(System
				.getenv(Environment.LOCAL_DIRS.name()));
		...
		if (workDir == null) {
			...
			try {
				...
				// Since all tasks will be running in their own JVM, the race
				// condition
				// exists where multiple tasks could be trying to create this
				// directory
				// at the same time. If this task loses the race, it's okay
				// because
	...
	private static void configureTask(JobConf job, Task task,
		...
		ApplicationAttemptId appAttemptId = ConverterUtils.toContainerId(
		...
		task.setShuffleSecret(JobTokenSecretManager
				.createSecretKey(shuffleSecret));
		...
		// Overwrite the localized task jobconf which is linked to in the
		// current
		// work-dir.
